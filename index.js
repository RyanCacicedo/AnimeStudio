...
async function renderAnime(title) {
    ...
    const anime = animes["data"].slice(0, 9).map((anime) => {
        return `<div class="movie">
        <figure class="movie__img--wrapper">
        <img src="${anime.images.jpg.large_image_url}" alt="" class="movie__img">
        <a href="${anime.url}" class="img__link" target="_blank"></a>
        </figure>
        <div class= "anime__organizer">
        <h3 class="movie__title">${anime.title} ${animeYear(anime.year)}<br> <a href="${anime.url}" class="anime__link" target="_blank"><i class="fa-solid fa-arrow-up-right-from-square"></i></a></h3>
